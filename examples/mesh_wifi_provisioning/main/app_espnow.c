        ...
        switch (evt.id) {
            case ESPNOW_RECV_CB:
...
#if ESPNOW_DEBUG
                ESP_LOGW(TAG, "Receive broadcast data from: "MACSTR", len: %d, self level: %d, recv_seq: %"PRIu32", current_seq: %"PRIu32"",
                ...
                if (espnow_msg_mode) {
                    ...
                    cJSON *led_status_js = cJSON_GetObjectItem(light_js, ESPNOW_LED_STATUS);
                    if (led_status_js) {
                        if (led_status_js->valueint) {
                            led_on();
                        } else {
                            led_off();
                        }
...
esp_err_t app_espnow_control_led_status(bool led_status)
{
    esp_err_t ret = ESP_FAIL;
    ...
    if (object) {
        if (payload) {
            if (led_status) {
                cJSON_AddTrueToObject(payload, ESPNOW_LED_STATUS);
            } else {
                cJSON_AddFalseToObject(payload, ESPNOW_LED_STATUS);
            }
